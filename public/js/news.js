async function fetchNews() {
  ...
  try {
    const response = await $.ajax({
      url: `https://newsapi.org/v2/everything?q=stock OR stocks OR equity&from=${START_DATE}&to=${END_DATE}&sortBy=publishedAt&apiKey=${API_KEY}`,
      method: 'GET',
    });
...
function displayNews(articles) {
  ...
  // 최신 주식 뉴스 섹션에 뉴스 추가
  const stockNewsSection = $('#stock-news-section'); // id를 사용하여 정확한 위치를 선택
